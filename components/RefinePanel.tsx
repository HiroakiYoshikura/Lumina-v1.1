...
import { Send, Image as ImageIcon, Loader2, Bot, User, Zap, Sparkles } from 'lucide-react';
...
export const RefinePanel: React.FC<Props> = ({ initialImage }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900/50 border border-gray-800 rounded-2xl backdrop-blur-sm overflow-hidden">
      {/* Header */}
              ...
              {msg.images && msg.images.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {msg.images.map((img, imgIdx) => (
                    <div key={imgIdx} className="relative group rounded-lg overflow-hidden border border-gray-700">
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <a 
                            ...
                            className="px-3 py-1 bg-white text-black rounded-full text-xs font-bold"
                        >
                            保存 (DL)
                        </a>
                      </div>
         ...
         {uploadedFiles.length > 0 && (
            <div className="flex items-center gap-2 mb-2 overflow-x-auto">
               ...
               {uploadedFiles.map((f, i) => (
                  <div key={i} className="text-xs bg-gray-800 border border-gray-700 px-2 py-1 rounded text-gray-300 flex items-center gap-1 whitespace-nowrap">
