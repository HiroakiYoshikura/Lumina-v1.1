...
import { ModelType, PresetConfig } from '../types';
...
export const generateContent = async (
  ...
  if (preset.model === ModelType.IMAGEN_4) {
    try {
  ...
  // --- ROUTE B: GEMINI (Generation + Reasoning) ---
    ...
    try {
      ...
      const selectedModel = useEconomyMode ? ModelType.GEMINI_2_5_FLASH : ModelType.GEMINI_3_PRO;
...
export const refineContent = async (
  ...
  useEconomyMode: boolean = false
    ...
    const contents = history.map(h => ({
        role: h.role,
        parts: [{ text: h.text || (h.images ? "画像が生成されました" : "") }]
    }));
